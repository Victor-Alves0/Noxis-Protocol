...
| `wallet keystore candidate header / NXKS / v1` | Public 100-byte header for a future encrypted wallet-root container; no secret payload or filesystem API exists. | Exact 100 bytes. | BE fields; fixed Argon2id/XChaCha candidate profile, salt, nonce, public wallet ID and key epoch. The exact header is associated data only in a test-only synthetic-root fixture. | Strict exact decoder rejects every different magic/version/algorithm/cost/zero wallet ID/trailing byte. Promotion requires file-lifecycle, recovery and independent-review gates. | [`WALLET_KEYSTORE_CONTAINER_CANDIDATE_V0_1.md`](WALLET_KEYSTORE_CONTAINER_CANDIDATE_V0_1.md); `noxis-wallet-keystore`. |
