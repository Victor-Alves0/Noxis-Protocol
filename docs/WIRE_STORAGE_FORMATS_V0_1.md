...
| `local experimental wire / NXPT / v1` | Private-transfer research packet, not a `NOXT` transaction. | Intent = 640 bytes; two envelopes = 1..4 KiB each; proof = 1..8,454,144 bytes, matching the bounded local `NXPP v1` envelope. | BE framing/lengths and EOF. **Frame domain: none**; no selected proof/KEM/AEAD binding. | Strict bounded parser; candidate local packet admission first verifies `NXRE` digest bindings then treats this opaque field as `NXPP`. Promotion requires candidate activation. | [`PRIVATE_TRANSFER_ARCHITECTURE_DRAFT_V0_1.md`](PRIVATE_TRANSFER_ARCHITECTURE_DRAFT_V0_1.md); `noxis-codec`. |
...
| `candidate private-submission journal / NXPL / v2` | Local composite receipt/state frame used only by `PrivateSubmissionStoreV2`; it retains a local `NXPP` byte-ID and transition facts, never the envelope/proof/witness itself. | Frame payload ≤ `396 + NXPR_MAX`; CRC-32 outer frame. | BE outer frame, sequence, IDs and `NXPR` length; input nullifiers/output commitments are canonical BabyBear `u32le`. The nested `NXPR` is rebuilt and its `StateId`, exactly two appended commitments, exactly two newly spent nullifiers and predecessor asset are all rechecked. | v1 and v2 readers reject one another; no mixed journal is accepted. A corrupt complete frame fails closed; only a structurally plausible final partial v2 frame may be re-scanned and truncated. Migration from v1 is not implemented. Candidate local storage only; no proof replay, consensus or network-history claim. | [`PRIVATE_SUBMISSION_HISTORY_ATOMICITY_DECISION_V0_1.md`](PRIVATE_SUBMISSION_HISTORY_ATOMICITY_DECISION_V0_1.md); `noxis-storage`. |
...
| `candidate local proof envelope / NXPP / v1` | Three pinned-research P3 proof chunks bound to `NXPD` and a supplied `NXPU`; local proof reconstruction only, not a transaction or selected verifier. | Proof chunks ≤ 8 MiB aggregate; framing/metadata = 244 bytes; total ≤ 8,454,144 bytes. | BE structural lengths, canonical BabyBear `u32le` input commitments and SHA-256 checksum `NOXIS/CANDIDATE-PRIVATE-PROOF-BUNDLE-ENVELOPE-CHECKSUM/V1\0`; exact deployment/statement binding and EOF. | Strict framing checks precede P3 deserialization; `decode_and_verify` additionally verifies all three proofs against supplied current state before returning a bundle. Promotion requires adversarial resource/fuzz review and candidate activation. | [`PRIVATE_PROOF_BUNDLE_ENVELOPE_CANDIDATE_V0_1.md`](PRIVATE_PROOF_BUNDLE_ENVELOPE_CANDIDATE_V0_1.md); `noxis-private-proof-contract`. |
