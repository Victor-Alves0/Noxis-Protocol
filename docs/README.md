...
| **How will backup and rollback be handled?** | [Backup and rollback policy candidate](WALLET_BACKUP_ROLLBACK_POLICY_CANDIDATE_V0_1.md) | It explains why an in-directory counter is insufficient, binds the synthetic `NXKP` payload to a strict public `NXKA` receipt, and leaves real backup/restore blocked until file-lifecycle tests and review exist. |
