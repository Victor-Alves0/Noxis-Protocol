...
| **What is already checked for the private-transfer research path?** | [Typed private-ledger admission](PRIVATE_LEDGER_ADMISSION_RESEARCH_V0_1.md), [its runnable local demo](PRIVATE_LEDGER_LOCAL_DEMO_RESEARCH_V0_1.md), [canonical private-state record](PRIVATE_STATE_RECORD_CANDIDATE_V0_1.md), [candidate private-state store](PRIVATE_STATE_STORE_CANDIDATE_V0_1.md), [private journal design](PRIVATE_STATE_JOURNAL_DESIGN_V0_1.md), [retained in-memory proof bundle](PRIVATE_TRANSFER_PROOF_BUNDLE_RESEARCH_V0_1.md), [complete local preflight](STARK_PRIVATE_TRANSFER_PREFLIGHT_RESEARCH_V0_1.md) and [local state transition](PRIVATE_STATE_TRANSITION_CANDIDATE_V0_1.md) | One optimized local path now proves `H_INTENT`, four private `H_NOTE` openings, asset equality and value conservation; retains that proof plus both ownership proofs; verifies all three against current state; atomically spends both 64-byte nullifiers and appends both outputs; derives a new anchor; and rejects replay without another mutation. `NXPR v1` now strictly preserves and rebuilds that local candidate state, and its single-writer store publishes/reopens one complete snapshot. The journal design is deliberately not implemented yet. It is not a wallet, ABCI or consensus store. Then follow the [local semantic preflight](PRIVATE_TRANSFER_LOCAL_PREFLIGHT_V0_1.md), [intent commitment candidate](PRIVATE_TRANSFER_INTENT_COMMITMENT_CANDIDATE_V0_1.md), [private state snapshot](PRIVATE_STATE_SNAPSHOT_CANDIDATE_V0_1.md), [typed NXSM anchor v2](PRIVATE_STATE_NXSM_ANCHOR_CANDIDATE_V0_1.md), [NXSM transition](PRIVATE_TRANSFER_NXSM_TRANSITION_CANDIDATE_V0_1.md), its [witness interface](PRIVATE_TRANSFER_WITNESS_CONSTRAINT_INTERFACE_CANDIDATE_V0_1.md) and the [unified public statement](PRIVATE_TRANSFER_PUBLIC_STATEMENT_CANDIDATE_V0_1.md). |
