...
| **Can I run the hybrid recipient components?** | [Local hybrid wallet-crypto demo](WALLET_CRYPTO_LOCAL_DEMO_V0_1.md) and [local candidate-note receipt](WALLET_PRIVATE_NOTE_RECEIPT_LOCAL_V0_1.md) | The commands demonstrate experimental key generation, strict `NXPA`/`NXRE` decoding, authenticated recipient encryption, hybrid identity signatures, a public-address catalog, and a recipient-side `H_NOTE` commitment recheck; they are not a wallet or private transfer. |
