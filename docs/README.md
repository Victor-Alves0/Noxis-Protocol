...
| **How is a receiving address paired with a note recipient commitment?** | [Local recipient descriptor candidate](RECIPIENT_DESCRIPTOR_LOCAL_CANDIDATE_V0_1.md) | A locally signed descriptor pairs `NXPA` and `H_ADDR`, and the wallet rejects decrypted notes for another commitment; it does not prove common secret derivation or provide a public descriptor protocol. |
