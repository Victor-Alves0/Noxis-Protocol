...
| **What is already checked for the private-transfer research path?** | [Retained in-memory proof bundle](PRIVATE_TRANSFER_PROOF_BUNDLE_RESEARCH_V0_1.md), [complete local preflight](STARK_PRIVATE_TRANSFER_PREFLIGHT_RESEARCH_V0_1.md), [intent-bound four-note value-conservation STARK](STARK_VALUE_CONSERVATION_RESEARCH_V0_1.md) and [local state transition](PRIVATE_STATE_TRANSITION_CANDIDATE_V0_1.md) | One executable local proof run first proves, in one AIR, `H_INTENT`, four private `H_NOTE` openings, public-asset equality, overflow-safe fixed-width `u128` conservation and the exact binding between intent output slots and the two output notes. A new typed bundle retains that proof plus both input ownership proofs, binds every relation to the same statement and input slot, and lets a fail-closed adapter verify them again against current state. It remains local and has no proof encoding or ledger admission. A separate transparent local transition derives the post-note root and `NXSM` root. Then follow the [local semantic preflight](PRIVATE_TRANSFER_LOCAL_PREFLIGHT_V0_1.md), [intent commitment candidate](PRIVATE_TRANSFER_INTENT_COMMITMENT_CANDIDATE_V0_1.md), [private state snapshot](PRIVATE_STATE_SNAPSHOT_CANDIDATE_V0_1.md), [typed NXSM anchor v2](PRIVATE_STATE_NXSM_ANCHOR_CANDIDATE_V0_1.md), [NXSM transition](PRIVATE_TRANSFER_NXSM_TRANSITION_CANDIDATE_V0_1.md), its [witness interface](PRIVATE_TRANSFER_WITNESS_CONSTRAINT_INTERFACE_CANDIDATE_V0_1.md) and the [unified public statement](PRIVATE_TRANSFER_PUBLIC_STATEMENT_CANDIDATE_V0_1.md). |
