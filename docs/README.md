...
| **Can I run the hybrid recipient components?** | [Local hybrid wallet-crypto demo](WALLET_CRYPTO_LOCAL_DEMO_V0_1.md), [local candidate-note receipt](WALLET_PRIVATE_NOTE_RECEIPT_LOCAL_V0_1.md) and [candidate envelope digest](RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md) | The commands demonstrate experimental key generation, strict `NXPA`/`NXRE` decoding, authenticated recipient encryption, hybrid identity signatures, a public-address catalog, a recipient-side `H_NOTE` commitment recheck and a candidate digest bound to output slot/commitment/envelope; they are not a wallet or private transfer. |
...
| **How are recipient keys derived, and where is the view key?** | [Recipient-root derivation candidate](RECIPIENT_ROOT_DERIVATION_CANDIDATE_V0_1.md) | It defines the executable local HKDF derivation and its tests. It also records that an exportable incoming/outgoing view key and keystore do not exist yet. |
