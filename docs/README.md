...
| **How do I move a local private-state store from v1 to v2?** | [Private-store migration operation](PRIVATE_SUBMISSION_MIGRATION_OPERATION_V0_1.md) | It is an explicit offline copy to a fresh target, preserving the v1 source and creating no invented historic receipts. |
...
| **How is the private proof envelope parser fuzzed?** | [NXPP parser-fuzzing guide](../fuzz/README.md) | A Linux CI smoke campaign fuzzes only bounded envelope framing, before P3 proof deserialization; crash artifacts are handled privately under `SECURITY.md`. |
...
| **How are recipient keys derived, and where is the view key?** | [Wallet-root session candidate](WALLET_ROOT_SESSION_LOCAL_CANDIDATE_V0_1.md) | A local secret root derives distinct recipient indexes; the [root derivation](RECIPIENT_ROOT_DERIVATION_CANDIDATE_V0_1.md) and [incoming view key](INCOMING_VIEW_KEY_LOCAL_CANDIDATE_V0_1.md) define the no-spend read authority and its bounded in-memory batch scanner. Neither can yet be persisted or exported. |
| **Can I reject a private packet with swapped envelopes?** | [Candidate packet envelope validation](PRIVATE_PACKET_ENVELOPE_VALIDATION_CANDIDATE_V0_1.md) | It strictly decodes a local `NXPT` plus its two `NXRE` values, checks their candidate P24 digests against the intent, then can hand only those bound outputs to an incoming view key. It is not proof, ledger or network validation. |
| **Does the runnable private path use a complete packet?** | [Local private-ledger demo](PRIVATE_LEDGER_LOCAL_DEMO_RESEARCH_V0_1.md), [proof-envelope admission](PRIVATE_PROOF_BUNDLE_ENVELOPE_CANDIDATE_V0_1.md) | Yes, locally: it creates two fresh `NXRE` envelopes, binds their digests into one `NXPT`, carries the bounded `NXPP` proof in that packet, validates the bindings, then verifies and admits it. This remains research software, not a wallet, ABCI or consensus transaction. |
| **What must happen before a wallet stores secrets?** | [Wallet keystore security gate](WALLET_KEYSTORE_SECURITY_GATE_V0_1.md) | The isolated [candidate container](WALLET_KEYSTORE_CONTAINER_CANDIDATE_V0_1.md) parses public `NXKS` metadata; the separate [synthetic payload](WALLET_KEYSTORE_PAYLOAD_SYNTHETIC_CANDIDATE_V0_1.md) exercises ciphertext binding without accepting a user secret file. |
| **How will backup and rollback be handled?** | [Backup and rollback policy candidate](WALLET_BACKUP_ROLLBACK_POLICY_CANDIDATE_V0_1.md) | It explains why an in-directory counter is insufficient and binds immutable synthetic `NXKP` files to a strict public `NXKA` receipt. The separate [portable synthetic bundle](WALLET_SYNTHETIC_RECOVERY_BUNDLE_CANDIDATE_V0_1.md) restores between directories without embedding that receipt; real secret backup/restore remains blocked. |
...
| **What is already checked for the private-transfer research path?** | [Typed private-ledger admission](PRIVATE_LEDGER_ADMISSION_RESEARCH_V0_1.md), [its runnable local demo](PRIVATE_LEDGER_LOCAL_DEMO_RESEARCH_V0_1.md), [canonical private-state record](PRIVATE_STATE_RECORD_CANDIDATE_V0_1.md), [candidate private-state store](PRIVATE_STATE_STORE_CANDIDATE_V0_1.md), [implemented private journal](PRIVATE_STATE_JOURNAL_DESIGN_V0_1.md), [retained in-memory proof bundle](PRIVATE_TRANSFER_PROOF_BUNDLE_RESEARCH_V0_1.md), [candidate proof transport budget](PRIVATE_PROOF_TRANSPORT_BUDGET_CANDIDATE_V0_1.md), [candidate local proof envelope](PRIVATE_PROOF_BUNDLE_ENVELOPE_CANDIDATE_V0_1.md), [local submission receipt](PRIVATE_PROOF_SUBMISSION_RECEIPT_CANDIDATE_V0_1.md), [submission-history atomicity decision](PRIVATE_SUBMISSION_HISTORY_ATOMICITY_DECISION_V0_1.md), [complete local preflight](STARK_PRIVATE_TRANSFER_PREFLIGHT_RESEARCH_V0_1.md) and [local state transition](PRIVATE_STATE_TRANSITION_CANDIDATE_V0_1.md) | One optimized local path now proves `H_INTENT`, four private `H_NOTE` openings, asset equality and value conservation; retains that proof plus both ownership proofs; serializes the retained bundle as local `NXPP v1` bytes; reconstructs and verifies all three proofs against current state; atomically spends both 64-byte nullifiers and appends both outputs; derives a new anchor; and rejects replay without another mutation. Successful admission returns a local envelope-ID receipt. `NXPR v1` strictly preserves and rebuilds candidate state; the legacy `NXPL v1` store recovers only post-state, while `PrivateSubmissionStoreV2` writes and recovers a single `NXPL v2` frame containing the local receipt facts plus that post-state. Neither path stores proofs or creates a public transaction ID. This is not a wallet, ABCI or consensus store. Then follow the [local semantic preflight](PRIVATE_TRANSFER_LOCAL_PREFLIGHT_V0_1.md), [intent commitment candidate](PRIVATE_TRANSFER_INTENT_COMMITMENT_CANDIDATE_V0_1.md), [private state snapshot](PRIVATE_STATE_SNAPSHOT_CANDIDATE_V0_1.md), [typed NXSM anchor v2](PRIVATE_STATE_NXSM_ANCHOR_CANDIDATE_V0_1.md), [NXSM transition](PRIVATE_TRANSFER_NXSM_TRANSITION_CANDIDATE_V0_1.md), its [witness interface](PRIVATE_TRANSFER_WITNESS_CONSTRAINT_INTERFACE_CANDIDATE_V0_1.md) and the [unified public statement](PRIVATE_TRANSFER_PUBLIC_STATEMENT_CANDIDATE_V0_1.md). |
| **Can the STARK preflight use validated packet envelopes?** | [Packet-bound STARK preflight](PACKET_BOUND_STARK_PREFLIGHT_RESEARCH_V0_1.md) | One release test first validates both `NXRE` values against `NXPT` digests, then runs the intent-bound conservation/ownership/output relations using that exact intent. It is still local research, not one transfer proof. |
| **What is the executable proof work?** | [public `H_INTENT` STARK relation](STARK_INTENT_COMMITMENT_RESEARCH_V0_1.md), [private `H_ADDR` STARK relation](STARK_H_ADDR_RESEARCH_V0_1.md), [private `H_NOTE` STARK relation](STARK_H_NOTE_RESEARCH_V0_1.md), [`H_NOTE` asset binding](STARK_H_NOTE_ASSET_BINDING_RESEARCH_V0_1.md), [intent-bound four-note value conservation](STARK_VALUE_CONSERVATION_RESEARCH_V0_1.md), [private ownership-and-depth-32-membership binding](STARK_NOTE_OWNERSHIP_BINDING_RESEARCH_V0_1.md), [private `NXSM` segments and local full-path preflight](STARK_NXSM_PREFIX_RESEARCH_V0_1.md), [anchored ownership preflight](STARK_ANCHORED_OWNERSHIP_PREFLIGHT_RESEARCH_V0_1.md), and the [verifier-profile boundary](STARK_VERIFIER_PROFILE_RESEARCH_V0_1.md) | `H_INTENT` proves the public candidate statement frame. The composed intent/value relation binds that frame's two output slots to private `H_NOTE` openings, public asset bytes and nonzero, overflow-safe `u128` conservation. Full-depth ownership and membership are one separate P24 research relation. The `NXSM` experiment can locally sequence 64 private segments to a full candidate root. A test-only proof serialization round trip verifies with a fresh configuration using the same pinned code profile; none of these components is aggregation, portable verification, or a transferable nullifier-absence or transfer proof. |
| **Why not one huge transfer AIR yet?** | [Composition resource decision](STARK_COMPOSITION_RESOURCE_DECISION_V0_1.md) | A verified one-input composed proof took 1,178.05 seconds. The project will keep independently auditable inner relations and design a versioned multi-table/recursive composition boundary before adding the second full Merkle path. |
| **What is the SP1 research path?** | [Recursive backend decision](RECURSIVE_BACKEND_DECISION_V0_1.md), [P24 depth-32 membership spike](SP1_P24_MEMBERSHIP_SPIKE_V0_1.md) | An isolated `no_std` P24 kernel is evaluated in SP1 against the existing reference implementation. It is research evidence only: it neither selects SP1 nor constitutes a transfer, a wire-proof format or a consensus rule. |
| **How do deployed privacy systems assemble transactions and proofs?** | [Private-transaction reference architectures](PRIVATE_TRANSACTION_REFERENCE_ARCHITECTURES_V0_1.md) | Source-level comparison of Orchard, Penumbra, Aztec and SP1, with the resulting Noxis decision: independently bound action proofs first, recursive compression later. |
...
7. [Typed private-ledger admission](PRIVATE_LEDGER_ADMISSION_RESEARCH_V0_1.md), [retained in-memory proof bundle](PRIVATE_TRANSFER_PROOF_BUNDLE_RESEARCH_V0_1.md), [complete sequential STARK preflight](STARK_PRIVATE_TRANSFER_PREFLIGHT_RESEARCH_V0_1.md), [executable output-note preflight](STARK_OUTPUT_NOTES_PREFLIGHT_RESEARCH_V0_1.md) and [anchored ownership preflight](STARK_ANCHORED_OWNERSHIP_PREFLIGHT_RESEARCH_V0_1.md)
