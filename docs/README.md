...
| **What is the executable proof work?** | [Private `H_ADDR` STARK relation](STARK_H_ADDR_RESEARCH_V0_1.md), [private `H_NOTE` STARK relation](STARK_H_NOTE_RESEARCH_V0_1.md), [private ownership-and-depth-32-membership binding](STARK_NOTE_OWNERSHIP_BINDING_RESEARCH_V0_1.md), [private eight-level `NXSM` prefix](STARK_NXSM_PREFIX_RESEARCH_V0_1.md), and [anchored ownership preflight](STARK_ANCHORED_OWNERSHIP_PREFLIGHT_RESEARCH_V0_1.md) | Full-depth ownership and membership are one P24 research relation. The preflight sequentially checks both candidate inputs against one `NXPS v2` anchor and locally revalidates `NXSM`; its new private prefix component is not full absence, aggregation or a transfer proof. |
