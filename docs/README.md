...
| **What must happen before a wallet stores secrets?** | [Wallet keystore security gate](WALLET_KEYSTORE_SECURITY_GATE_V0_1.md) | The isolated [candidate container](WALLET_KEYSTORE_CONTAINER_CANDIDATE_V0_1.md) strictly parses public `NXKS` metadata and tests synthetic sealing; no user secret file exists today. |
| **How will backup and rollback be handled?** | [Backup and rollback policy candidate](WALLET_BACKUP_ROLLBACK_POLICY_CANDIDATE_V0_1.md) | It explains why an in-directory counter is insufficient and implements a strict public `NXKA` receipt the user must retain independently before secret backup/restore may exist. |
