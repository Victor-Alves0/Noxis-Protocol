...
4. Dependência presa no `Cargo.lock`, verificada com Rust 1.93, e testes de parser/fuzzing e benchmark de memória/CPU com entradas adversariais.
...
A avaliação inicial registrou a incompatibilidade de Plonky3 com Rust 1.85 e a
linha antiga que não fecha testes publicados; detalhes em
[`POSEIDON2_CANDIDATE_EVALUATION_V0_1.md`](POSEIDON2_CANDIDATE_EVALUATION_V0_1.md).
O MSRV atual é 1.93 para tornar o experimento STARK reproduzível, sem selecionar
Plonky3 como backend de árvore. A referência Rust Horizen e a implementação
Zig independente já reproduziram dois vetores de permutação, registrados em
[`POSEIDON2_BABYBEAR16_REFERENCE_EVALUATION_V0_1.md`](POSEIDON2_BABYBEAR16_REFERENCE_EVALUATION_V0_1.md).
O artigo [Poseidon2](https://eprint.iacr.org/2023/323.pdf) orienta a família
de construção, mas não fornece os parâmetros próprios do Noxis.
