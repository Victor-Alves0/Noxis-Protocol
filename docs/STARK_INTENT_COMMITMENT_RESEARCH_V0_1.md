...
object round trip through `postcard` verifies with a fresh configuration using
the same pinned code profile; it has no portable verifier profile, Noxis proof
serialization or production activation. See
