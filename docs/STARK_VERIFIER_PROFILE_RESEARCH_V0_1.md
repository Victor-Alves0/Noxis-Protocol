...
Its local byte representation has a strict decoder: it accepts only the
single byte-for-byte canonical descriptor generated by the pinned research
code. This is an audit boundary for independently recorded local metadata, not
a new wire or storage format.
