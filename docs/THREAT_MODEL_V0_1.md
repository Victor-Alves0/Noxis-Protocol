...
| State rollback, crash, or concurrent update | Previously spent nullifier becomes spendable again, or state becomes inconsistent. | `NXCB` recovery re-executes a complete synchronized block frame before advancing the tip; an incomplete final frame is removed only after its complete prefix validates. `NXCP` is canonical and checked against legacy `NXRF` replay. The candidate private store holds an exclusive cooperative lock, publishes one complete `NXPR` snapshot through a synchronized temporary file and reopens it by reconstruction; it is not a private transition log or consensus durability. Runtime and journal locks serialize cooperating writers; consensus commits fail closed on platforms without the documented directory-durability barrier. | Add comprehensive fault injection, storage-platform validation, authenticated checkpoint acceleration, private transition history, replication, backup/restore and hostile multi-process tests. |
