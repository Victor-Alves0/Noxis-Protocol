...
| Node operator | Composes the modules and invokes ledger transitions. | Currently controls availability and verifier/policy selection; consensus is the chosen future architecture, but no implementation constrains it yet. |
...
| Equivocation by node operator | Different clients observe different ledgers or verifier/policy choices. | No network or consensus implementation exists; the consensus model is selected and documented. | Select an engine and fault model; implement authenticated peers, finality, a vote WAL, and configuration governance. |
