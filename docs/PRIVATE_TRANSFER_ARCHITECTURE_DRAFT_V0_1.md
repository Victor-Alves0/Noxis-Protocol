...
O segundo componente entregue é o enquadramento externo `NXPT` v1, em `noxis-codec`. Ele não altera o formato `NOXT` v1 nem cria uma operação aceita pelo ledger. Sua ordem é fixa: `"NXPT"`, versão `u16` big-endian, intenção de 640 bytes, dois envelopes com comprimento `u16` e uma prova com comprimento `u32`. Cada envelope deve conter de 1 a 4 KiB; a prova deve conter de 1 byte a 8.454.144 bytes, exatamente o teto do envelope local `NXPP v1`. O decoder verifica tamanho antes de copiar bytes, recusa versões, truncamento e bytes finais. O limite é um orçamento local de transporte e não uma garantia de produção: ainda requer benchmark adversarial do backend escolhido.

O parser `NXPT` continua estrutural e não verifica prova. Porém a fronteira local `noxis-private-packet-validation` agora redecodifica os dois envelopes como `NXRE` canônicos, reencoda-os sem normalização e compara `H_ENVELOPE(slot, commitment, nxre)` aos dois digests da intenção antes de qualquer prova. A admissão local do ledger usa então somente a intenção daquele pacote para derivar `NXPU`, interpreta seu campo de prova como `NXPP v1`, verifica-o contra o estado atual e aplica a transição atomicamente. Isso não remove o gate do serviço: não há vetores externos, AIR/profilo verificador selecionado, wallet de usuário, ABCI ou consenso. A medição completa em release de 2026-09-02 foi 4.968.511 bytes de provas brutas: 2.836.034 para intenção/valor, 1.066.377 e 1.066.100 para as ownerships. Um [orçamento candidato separado](PRIVATE_PROOF_TRANSPORT_BUDGET_CANDIDATE_V0_1.md) limita 8 MiB de provas brutas e 64 KiB explícitos de framing/metadados, e o teto correspondente do `NXPT` permite esse `NXPP` local. Isso não encerra a revisão de negação de serviço, que ainda precisa de tamanhos adversariais e custo de verificação. Ver também [`STARK_VERIFIER_PROFILE_RESEARCH_V0_1.md`](STARK_VERIFIER_PROFILE_RESEARCH_V0_1.md), [`RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md`](RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md) e [`PRIVATE_PACKET_ENVELOPE_VALIDATION_CANDIDATE_V0_1.md`](PRIVATE_PACKET_ENVELOPE_VALIDATION_CANDIDATE_V0_1.md).
