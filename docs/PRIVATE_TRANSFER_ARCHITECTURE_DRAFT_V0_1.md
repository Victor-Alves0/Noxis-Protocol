...
O parser `NXPT` continua estrutural e não verifica prova. Porém a fronteira local `noxis-private-packet-validation` agora redecodifica os dois envelopes como `NXRE` canônicos, reencoda-os sem normalização e compara `H_ENVELOPE(slot, commitment, nxre)` aos dois digests da intenção antes de qualquer prova. Ainda não há vetores externos, AIR, seleção criptográfica ou chamada pelo ledger; por isso, o pacote permanece impossível de aplicar no ledger v1 e não remove o gate do serviço. A medição em release de 2026-09-02 mostrou que uma única prova P24 de ownership usa 1.066.268 bytes; o cap provisório de prova de 2 MiB não comporta nem as duas ownerships do bundle, antes da relação de intenção/valor. Logo, `NXPT v1` não pode transportar o bundle atual e seu orçamento só será substituído após medição completa e revisão de negação de serviço. Ver [`STARK_VERIFIER_PROFILE_RESEARCH_V0_1.md`](STARK_VERIFIER_PROFILE_RESEARCH_V0_1.md), [`RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md`](RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md) e [`PRIVATE_PACKET_ENVELOPE_VALIDATION_CANDIDATE_V0_1.md`](PRIVATE_PACKET_ENVELOPE_VALIDATION_CANDIDATE_V0_1.md).
