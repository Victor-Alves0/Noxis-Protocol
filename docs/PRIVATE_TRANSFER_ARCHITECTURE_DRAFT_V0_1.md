...
O `NXPT` ainda não verifica que um envelope corresponde ao seu digest público nem verifica a prova. Existe agora um cálculo local candidato de `H_ENVELOPE` que compromete slot, commitment e bytes `NXRE` canônicos, mas o parser de pacote ainda não o recompõe e não há vetores externos, AIR ou seleção criptográfica. Essa ligação continua responsabilidade de uma fronteira verificadora posterior; por isso, o pacote é deliberadamente impossível de aplicar no ledger v1 e não remove o gate do serviço. Ver [`RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md`](RECIPIENT_ENVELOPE_DIGEST_CANDIDATE_V0_1.md).
