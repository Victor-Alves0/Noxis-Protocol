...
`CommitmentV2`, `NullifierV2` e `MerkleRootV2` usam codificação total de dois elementos de campo (64 bytes). É quebra de formato deliberada: codec, record-chain, storage, checkpoint e gênese v1 ficam imutáveis; uma rede v2 começa com nova gênese.
...
O `NXPT` ainda não verifica que um envelope corresponde ao seu digest público nem verifica a prova. Essa ligação será responsabilidade do backend criptográfico quando forem definidos hash, KEM/AEAD e AIR. Por isso, o pacote é deliberadamente impossível de aplicar no ledger v1 e não remove o gate do serviço.
